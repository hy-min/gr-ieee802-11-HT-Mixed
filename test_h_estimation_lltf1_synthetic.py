...
def test_lltf0_h_residual_phase():
    ...
    rotates 50.0 rad/us (large enough to expose a clear residual).
    ...
    H_chan_t_lsig = rotate_per_symbol(H_chan_t0, 50.0, t_us=8.0)
    lsig_rx = bpsk * H_chan_t_lsig
    eq_lltf0 = lsig_rx / H_lltf0

    imag_max_lltf0 = np.max(np.abs(eq_lltf0.imag))
    print(f"[L-LTF0 H] imag_max={imag_max_lltf0:.4f} (expect LARGE for time-varying channel)")
...
def test_lltf1_h_residual_phase():
    ...
    H_chan_t_lltf1 = rotate_per_symbol(H_chan_t0, 50.0, t_us=4.0)
