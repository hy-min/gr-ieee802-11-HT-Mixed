...
7. **物理改进永远低优先级**：先穷尽软件路径（新算法/拟合方法/架构），再考虑
   硬件改动（换线/屏蔽/共享参考/换设备/拉距离）。教训：P165c 推翻 P150
   "LO 墙"（物理墙结论下早）；2026-08-18 离群帧的拟合鲁棒化（Theil-Sen/
   迭代剔除）未探索前"软件穷尽"即过早结论——物理路径只在软件路径
   逐项排除后才进入讨论。
